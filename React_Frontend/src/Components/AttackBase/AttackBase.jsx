...
import cannonTowerAmmo from '../../BuildingsGallery/cannonTowerAmmo.jpg'
...
export default class AttackBase extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    cannonBallMover(building, xCoordinate, yCoordinate, distance, startDistance){
        if(this.state.buildings.find(building => building.xCoordinate === xCoordinate + 1 && building.yCoordinate === yCoordinate)){
            console.log("siin2")
    ...
    coordinatesToViewID(xCoordinate, yCoordinate){
    ...
    handleKeyPress(e){
        ...
        if(e.key === "w" && this.state.buildings.find(building => building.xCoordinate === this.state.activeHero.xCoordinate - 1 && building.yCoordinate === this.state.activeHero.yCoordinate) === undefined){
            if(0 <= this.state.activeHero.xCoordinate - 1){
                this.state.activeHero.xCoordinate = this.state.activeHero.xCoordinate - 1;
            }
        }
        else if(e.key === "a" && this.state.buildings.find(building => building.xCoordinate === this.state.activeHero.xCoordinate && building.yCoordinate === this.state.activeHero.yCoordinate - 1) === undefined){
            if(0 <= this.state.activeHero.yCoordinate - 1) {
                this.state.activeHero.yCoordinate = this.state.activeHero.yCoordinate - 1;
            }
        }
        else if(e.key === "s" && this.state.buildings.find(building => building.xCoordinate === this.state.activeHero.xCoordinate + 1 && building.yCoordinate === this.state.activeHero.yCoordinate) === undefined){
            if(this.state.activeHero.xCoordinate + 1 <= 11) {
                this.state.activeHero.xCoordinate = this.state.activeHero.xCoordinate + 1;
            }
        }
        else if(e.key === "d" && this.state.buildings.find(building => building.xCoordinate === this.state.activeHero.xCoordinate && building.yCoordinate === this.state.activeHero.yCoordinate + 1) === undefined){
